...
> 适用对象：小CEO 林兴明（本地服务器：已备案域名 + GitHub 账号 `luoduo-ai-army`）
> 执行产物：本仓库 `submissions/luoduo-ai-army/luoduo-agent-city/`
...
- **本沙箱无 GitHub token，无法直推 PR。** 以下所有 `git push` / 开 PR 操作必须由小CEO在**自有本地服务器**（已登录 GitHub 账号 `luoduo-ai-army`）完成。
- 本仓库已在 `E:\北京参赛\` 完成 `git init` 与首次提交（`ready_for_review` 状态），提交包内的 `.py` 辅助脚本已通过 `.gitignore` 排除，不会进入提交。
...
方式 A（GitHub 网页）：打开 `https://github.com/open-city-ai/haidian` → 右上角 **Fork** → 选择账号 `luoduo-ai-army`。
...
# 或手动 clone 你的 fork：
git clone https://github.com/luoduo-ai-army/haidian.git
cd haidian
```
...
cp -r /path/to/北京参赛/submissions/luoduo-ai-army/luoduo-agent-city \
      ~/haidian/submissions/luoduo-ai-army/luoduo-agent-city
```

复制后请核对 fork 内目录含：`manifest.json`、`proposal.md`、`*.json`、`geometry/`、`assets/figures/`、`report/`、`visual/`、`drawings/`，**不含**任何 `.py` 辅助脚本。
...
python3 scripts/self_check_submission.py submissions/luoduo-ai-army/luoduo-agent-city
...
然后在 GitHub 网页对 `luoduo-ai-army/haidian` 点 **Contribute / Open pull request**，目标：
- **base repository**: `open-city-ai/haidian`
- **base**: `main`
- **head**: `luoduo-ai-army/haidian:main`（或你的特性分支）
